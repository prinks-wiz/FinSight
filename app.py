...
def analyze_stock_data(stock_data):
  ...
  greatest_increase_pct = round(((greatest_increase['Close'] - greatest_increase['Open']) / greatest_increase['Open'] * 100),3)

  # Greatest decrease
  greatest_decrease = last_month_data.sort_values(by='Close', ascending=True).iloc[0]
  greatest_decrease_pct = round(((greatest_decrease['Open'] - greatest_decrease['Close']) / greatest_decrease['Open'] * 100),3)

  # Current close value
  current_close = round((df.iloc[-1]['Close']),3)

  # Volatility (average daily fluctuation over the last week)
  last_week_data = df[df['Date'] >= df['Date'].iloc[-1] - pd.Timedelta(days=7)]
  volatility = round((last_week_data['Close'].pct_change().std() * 100),3)

  # Moving average of the stock over 1 month
  moving_average_1_month = round((df['Close'].rolling(window=30).mean().iloc[-1]),3)
...
def process_sentiment_output(sentiment_output):
    articles = []
    for entry in sentiment_output:
        article = {
...
def search_news():
    ...
    sentiment = sentiment_analysis(headlines,article_links,descriptions)
    
    articles = process_sentiment_output(sentiment)
